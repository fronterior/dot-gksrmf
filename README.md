...
점은 **입력 모드 스위치**입니다. 프롬프트 맨 앞에 점을 하나 찍으면, 그 프롬프트 전체가 한글 입력으로 취급됩니다. 여러 줄을 쳐도 둘째 줄부터 다시 찍을 필요가 없습니다.
...
점이 없는 프롬프트는 전혀 건드리지 않으므로, 평소의 영문 프롬프트와 한 세션에서 섞어 써도 안전합니다.

## 동작 방식

`UserPromptSubmit` 훅이 프롬프트를 가로채, 점으로 시작하면 점을 뗀 나머지 전체를 두벌식 자판 기준으로 한글로 바꾼 뒤 그 결과를 Claude 의 컨텍스트에 주입합니다. Claude 는 영문 나열 대신 변환된 한글 문장을 요청으로 읽습니다.
...
한 문장 안에 영문이 섞인다고 자판을 바꿔야 한다면 목적이 무너집니다. 그래서 프롬프트 전체를 한글로 변환하되, 영문으로 남아야 하는 부분은 **영문 자판을 유지한 채로** 표시하거나 자동으로 가려냅니다.
